    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new Error('Access denied to this vendor data.');
        } else if (response.status === 404) {
          throw new Error('Vendor dashboard data not found.');
        }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">
                        {data.stats.totalContracts > 0 ? 
                          Math.round(((data.stats.totalContracts - data.stats.activeContracts) / data.stats.totalContracts) * 100) 
                          : 0}%
                      </span>
                    </div>
                  </div>
          ...
          <TabsContent value="documents">
            <VendorDocuments vendorId={data.vendor.id} />
          </TabsContent>

          <TabsContent value="messages">
            <VendorMessages vendorId={data.vendor.id} />
          </TabsContent>
