...
import type { AuditLog, AuditAction, UserRole } from '@/shared/types/vendor-portal';
...
export async function getAuditLogs(params: {
  ...
  const limit = Math.floor(Number(params.limit) || 100);
  const offset = Math.floor(Number(params.offset) || 0);
  
  // Clamp limit to safe maximum and ensure offset is non-negative
...
export async function getAuditSummary(params: {
    ...
    { $match: filter },
    {
      $facet: {
        ...
        riskEvents: [
          {
            $match: {
              action: { $in: ['login_failed', 'access_revoked', 'delete'] }
            }
          },
          { $count: 'count' }
        ]
  ...
  // Transform facet results into expected format
  const totalActions = result.totalActions[0]?.count || 0;
  
  const actionsByType: Record<string, number> = {};
  result.actionsByType.forEach((item: { _id: string; count: number }) => {
    actionsByType[item._id] = item.count;
  });
  
  const userActivity: Record<string, number> = {};
  result.userActivity.forEach((item: { _id: string; count: number }) => {
    userActivity[item._id] = item.count;
  });
  
  const resourceAccess: Record<string, number> = {};
  result.resourceAccess.forEach((item: { _id: string; count: number }) => {
    resourceAccess[item._id] = item.count;
  });
  
  const riskEvents = result.riskEvents[0]?.count || 0;
