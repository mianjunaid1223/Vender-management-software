...
async function addOverdueInvoices() {
  try {
    const db = await getDb();
    
    // Create test invoices with overdue dates
    const overdueInvoices: Partial<Invoice>[] = [
      {
    ...
    console.log(`Successfully inserted ${result.insertedCount} overdue invoices`);
    
    // List all invoices
    const allInvoices = await db.collection('invoices').find({}).toArray();
    console.log('\nAll invoices in database:');
    allInvoices.forEach(inv => {
      console.log(`- ${inv.invoiceNumber}: due ${inv.invoiceDueDate}, status: ${inv.status}`);
    });
